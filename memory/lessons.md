...
## 2026-08-22 — A documented "known issue" was itself stale; the fix was to check the live system, not to schedule work

`businesses/gaia/product-data.md` stated the GWW mobile Sheet mirror covered Oren Sport only, dated 2026-08-11. On that basis, a "known issue" was written and a decision logged to plan extending the mirror to the other three suppliers. Checking the live Sheet directly via the Composio `googlesheets` connection (not previously done — the file's claim had only ever been read, never re-verified) showed it already had full four-supplier coverage as of that same 2026-08-11 date; the sync had happened and the file was simply never updated afterward. The instinct to plan remediation work came before the instinct to verify the premise against the live system — backwards, given this repository's own stated rule that a live system wins over a file when they disagree. Check the live source before writing a known-issue or scheduling work to fix it, especially when a tool to check it (here, an already-connected Composio toolkit) is available in the same session.
